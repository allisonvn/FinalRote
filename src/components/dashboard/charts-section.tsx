...
export function ChartsSection({ className, experiments = [], stats, realtime }: ChartsSectionProps) {
  ...
  useEffect(() => {
    const loadMore = async () => {
      try {
        const { getExperimentMetrics, getDeviceBreakdown, getFunnelData, getPreviousPeriodMetrics } = await import('@/lib/analytics')
        const [metrics, devices, funnel, prev] = await Promise.all([
          getExperimentMetrics(timeRange),
          getDeviceBreakdown(timeRange, selectedExperiment !== 'all' ? selectedExperiment : undefined),
          getFunnelData(timeRange),
          getPreviousPeriodMetrics(timeRange, selectedExperiment !== 'all' ? selectedExperiment : undefined)
        ])
  ...
  // Calcular comparação temporal de improvement
  const previousPeriodImprovement = previousMetrics?.conversionRate || 0
  const improvementChange = avgImprovement - (previousPeriodImprovement > 0 ? ((avgImprovement - previousPeriodImprovement) / previousPeriodImprovement) * 100 : 0) // Simplificação
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <Badge className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-500/20 to-green-500/20 border-2 border-emerald-500/30 text-emerald-700 dark:text-emerald-300 font-bold shadow-lg">
                    ...
                    +{avgImprovement.toFixed(1)}% lift
                  </Badge>
                  <Button
                    size="sm"
                    onClick={() => setRefreshTrigger(prev => prev + 1)}
                    className="rounded-xl bg-white/50 hover:bg-white dark:bg-gray-800/50 dark:hover:bg-gray-800 backdrop-blur-sm shadow-lg transition-all duration-300 hover:scale-110"
                  >
                ...
                {significanceData.map((item, index) => (
                  <div key={index} className="group/item p-4 rounded-2xl bg-gradient-to-br from-purple-50/50 to-pink-50/50 dark:from-purple-950/20 dark:to-pink-950/20 hover:from-purple-100/80 hover:to-pink-100/80 dark:hover:from-purple-900/30 dark:hover:to-pink-900/30 border-2 border-purple-200/50 dark:border-purple-700/50 hover:border-purple-300 dark:hover:border-purple-600 transition-all duration-300 hover:scale-[1.02] shadow-lg">
                    ...
                    <div className="relative w-full bg-slate-200 dark:bg-slate-700 rounded-full h-4 overflow-hidden shadow-inner">
                      <div
                        className={`h-4 rounded-full transition-all duration-1000 ease-out relative overflow-hidden shadow-lg ${item.significance >= 95 ? 'bg-gradient-to-r from-emerald-500 to-green-500' :
